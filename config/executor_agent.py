...
def create_jira_stories(stories_file_path: str) -> str:
    """Create Jira stories from a JSON file."""
    try:
        logger.info(f"EXECUTOR: Creating Jira stories from {stories_file_path}")
        
        if not os.path.exists(stories_file_path):
            error_msg = f"Stories file not found at {stories_file_path}"
        ...
        if not stories:
            error_msg = "No stories found in file"
            logger.error(error_msg)
            return json.dumps({"status": "error", "message": error_msg})
        ...
        for story in stories:
            try:
                issue_key = create_jira_story(story)
                created_stories.append(issue_key)
                logger.info(f"Created Jira story: {issue_key}")
            except Exception as e:
                logger.error(f"Error creating story {story.get('summary')}: {str(e)}")
                
        result = {
            "status": "success",
            "message": f"Created {len(created_stories)} Jira stories",
            "created_stories": created_stories
        }
        logger.info(f"Successfully created {len(created_stories)} Jira stories")
        return json.dumps(result)
        
    except Exception as e:
        error_msg = f"Error creating Jira stories: {str(e)}"
        logger.error(error_msg)
        return json.dumps({"status": "error", "message": error_msg})
...
executor_agent = UserProxyAgent(
    ...
    function_map={
        ...
        "create_jira_stories": create_jira_stories
    },
