  ...
  async.waterfall([
    ...
    function (emailHTML, user, done) {
      ...
      smtpTransport.sendMail(mailOptions, function (err) {
        if (!err) {
          res.send(errorHandler.getJsonError("reset_message"));
        } else {
          return res.status(400).send(errorHandler.getJsonError("send_mail_fail"));
        }
...
exports.validateResetToken = function (req, res) {
  User.findOne({
    ...
    res.redirect('/password/reset/' + req.params.resettoken);
  });
};
